class Game {
  ...
  start() {//método para obter o gameState e então iniciar o jogo
    ...
    player = new Player();
    //inciando a variável playerCount //faltava esse ponto
  ...
  if(allPlayers !== undefined){
    ...
    for(var plr in allPlayers){
      ...
      var y = height - allPlayers[plr].positionY //aqui estava positionX
      index = index +1
  ...
  handleFuel(index){
    cars[index - 1].overlap(fuels,function(collector,colected) {
      player.fuel = 185
      colected.remove()
    });
  }
  handlePowerCoins(index){
    cars[index - 1].overlap(powerCoins,function(collector,colected) {
      player.score += 21
      player.update()
      colected.remove()
    });
